...
export const GroupEmulationView: React.FC<GroupEmulationViewProps> = ({
  ...
  // Base score configuration state (Synced with Class Emulation Summary)
  ...
  const groupSummaries = useMemo(() => {
    ...
    return ([1, 2, 3, 4] as const).map((groupNum) => {
      ...
      // 4. Duty Score: Đã hoàn thành (+0đ), Đang thực hiện (+0đ), Chưa bắt đầu (+0đ), Chưa hoàn thành (-5đ / 1 HS)
  ...
  return (
    <div className="space-y-6">
            ...
            {(role === 'gvcn' || role === 'csl') && (
              <>
                <button
                  ...
                  className="px-3.5 py-2.5 rounded-xl bg-white/20 hover:bg-white/30 text-white font-bold text-xs shadow-sm border border-white/30 active:scale-95 transition-all flex items-center gap-1.5 cursor-pointer whitespace-nowrap"
                  title="Cài đặt/Thay đổi điểm gốc ban đầu (0đ, 100đ hoặc tự chỉnh)"
                >
                  <SlidersHorizontal className="w-4 h-4 text-amber-200" />
                  <span>Điểm Gốc ({groupSummaries[0]?.baseScore ?? 0}đ)</span>
                </button>
                <button
                  onClick={() => setIsAddModalOpen(true)}
                  className="px-4 py-2.5 rounded-xl bg-white text-orange-800 font-bold text-xs shadow-md hover:bg-amber-50 active:scale-95 transition-all flex items-center gap-1.5 cursor-pointer whitespace-nowrap"
                >
                ...
                <button
                  ...
                  className="px-3 py-2.5 rounded-xl bg-red-500/90 hover:bg-red-600 text-white font-bold text-xs shadow-md active:scale-95 transition-all flex items-center gap-1.5 cursor-pointer whitespace-nowrap"
                  title="Xóa hết bản ghi điểm khen thưởng/phạt thi đua tổ để thiết lập lại"
                >
      ...
      <div className="bg-gradient-to-r from-blue-50/90 to-indigo-50/80 dark:from-blue-950/40 dark:to-indigo-950/40 border border-blue-200/80 dark:border-blue-800/80 rounded-2xl p-3.5 flex flex-wrap items-center justify-between gap-3 text-xs shadow-2xs">
        ...
        <div className="flex flex-wrap items-center gap-2 font-bold text-[11px]">
          ...
          <span className="bg-amber-100 dark:bg-amber-950/60 text-amber-900 dark:text-amber-300 px-2.5 py-1 rounded-xl border border-amber-300 dark:border-amber-700 flex items-center gap-1 shadow-2xs">
            🧹 <strong>Trực Nhật:</strong> Xong (+0đ) • Đang làm (+0đ) • Chưa làm (+0đ) • Chưa xong (<strong className="text-rose-600 font-black">-5đ/1HS</strong>)
          </span>
        </div>
      </div>
        ...
        <div className="lg:col-span-7 space-y-4">
          <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-slate-500 font-semibold">Thang điểm cơ sở {groupSummaries[0]?.baseScore ?? 0}đ</span>
            </div>
              ...
              {groupSummaries.map((g) => (
                <div key={g.group} className="space-y-1.5">
                  ...
                  <div className="w-full h-3.5 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden flex shadow-inner">
                    ...
                    {g.baseScore > 0 && (
                      <div
                        style={{ width: `${Math.min(g.baseScore * 0.4, 45)}%` }}
                        className="bg-slate-400 dark:bg-slate-600"
                        title={`Điểm gốc: ${g.baseScore}đ`}
                      />
                    )}
                    {/* Academic */}
            ...
            <div className="flex flex-wrap items-center gap-3 pt-3 border-t border-slate-100 dark:border-slate-800 text-[11px] text-slate-500">
              <span className="flex items-center gap-1.5 font-bold text-slate-700 dark:text-slate-300">
                <span className="w-2.5 h-2.5 rounded-full bg-slate-400" /> Điểm Gốc ({groupSummaries[0]?.baseScore ?? 0}đ)
              </span>
              <span className="flex items-center gap-1.5">
              ...
              {(role === 'gvcn' || role === 'csl') && (
                <div className="flex items-center gap-2">
                  ...
                  {emulationLogs.length > 0 && (
                    <button
                      onClick={() => setIsClearConfirmOpen(true)}
                      className="text-xs font-bold text-red-500 hover:underline flex items-center gap-1 cursor-pointer ml-1"
                      title="Xóa toàn bộ bản ghi thi đua tổ để thiết lập lại"
                    >
      ...
      {/* CONFIRM CLEAR ALL EMULATION LOGS MODAL */}
      {isClearConfirmOpen && (
        <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-md w-full border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 p-6 space-y-4">
            ...
            <div>
              <h3 className="text-base font-black text-slate-900 dark:text-white">
                Xác Nhận Xóa Hết Dữ Liệu Thi Đua
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-1.5 leading-relaxed">
                Bạn có chắc chắn muốn xóa toàn bộ <strong>{emulationLogs.length} bản ghi</strong> điểm thưởng / phạt thi đua phong trào của 4 tổ để thiết lập lại từ đầu không? Hành động này không thể hoàn tác.
              </p>
            </div>
            ...
            <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
              ...
              <button
                type="button"
                onClick={() => {
                  if (onClearAllEmulationLogs) {
                    onClearAllEmulationLogs();
                  }
                  setCustomLeaders({});
                  setIsClearConfirmOpen(false);
                }}
                className="px-4 py-2 rounded-xl bg-red-600 hover:bg-red-700 text-white font-black text-xs shadow-md flex items-center gap-1.5 cursor-pointer"
              >
      ...
      {/* Modal Cài Đặt Điểm Gốc */}
      {isBaseScoreModalOpen && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 shadow-2xl border border-slate-200 dark:border-slate-800 max-w-md w-full space-y-5 animate-in fade-in">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
              <h3 className="text-base font-bold text-[#003366] dark:text-white flex items-center gap-2">
                <SlidersHorizontal className="w-5 h-5 text-amber-500" />
                Cấu Hình Điểm Gốc Ban Đầu 4 Tổ
              </h3>
              <button onClick={() => setIsBaseScoreModalOpen(false)} className="text-slate-400 hover:text-slate-600">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-bold text-slate-900 dark:text-white focus:ring-2 focus:ring-amber-500"
                />
              </div>
              ...
              <div className="flex items-center justify-between pt-1">
                <span className="text-xs text-slate-500">Mẫu cài sẵn:</span>
                <div className="flex gap-2">
                  <button
                    ...
                    className="px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold text-xs cursor-pointer"
                  >
                    Mặc định 0đ
                  </button>
                  <button
            ...
            <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-100 dark:border-slate-800">
              <button
                ...
                className="px-4 py-2 rounded-xl border border-slate-200 text-slate-600 dark:text-slate-400 font-bold text-xs cursor-pointer"
              >
                Hủy
              </button>
              <button
                ...
                className="px-4 py-2 rounded-xl bg-gradient-to-r from-orange-600 to-amber-600 hover:from-orange-700 hover:to-amber-700 text-white font-bold text-xs shadow-md cursor-pointer active:scale-95"
              >
                Lưu & Đồng Bộ
              </button>
            </div>
