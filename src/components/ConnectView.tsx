...
import { getStoredGoogleSheetConfig, saveChatMessages } from '../lib/storage';
import {
...
export const ConnectView: React.FC<ConnectViewProps> = ({
  ...
  // Cloud Sheet Sync & Polling State
  const [isCloudSyncing, setIsCloudSyncing] = useState(false);
  const [lastSyncTime, setLastSyncTime] = useState<string | null>(null);

  const activeCloudConfig = useMemo(() => {
    return googleSheetConfig || getStoredGoogleSheetConfig();
  }, [googleSheetConfig]);

  const hasCloudConfig = Boolean(
    activeCloudConfig &&
    activeCloudConfig.sheetUrl &&
    validateAppsScriptUrl(activeCloudConfig.sheetUrl)
  );

  const handleManualSyncCloud = useCallback(async () => {
    if (!hasCloudConfig || !activeCloudConfig?.sheetUrl) return;
    setIsCloudSyncing(true);
    try {
      const cloudMsgs = await fetchChatMessagesFromCloud(
        activeCloudConfig.sheetUrl,
        classInfo?.className || '11D5'
      );
      if (Array.isArray(cloudMsgs) && cloudMsgs.length > 0) {
        ...
        if (addedCount > 0) {
          ...
          if (onUpdateMessages) {
            onUpdateMessages(merged);
          }
          showToast(`🔄 Đã đồng bộ ${addedCount} tin nhắn mới từ Cloud Sheet!`);
        }
      }
      setLastSyncTime(new Date().toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' }));
    } catch (err) {
      console.warn('Manual cloud chat sync failed:', err);
    } finally {
      setIsCloudSyncing(false);
    }
  }, [hasCloudConfig, activeCloudConfig, messages, classInfo?.className, onUpdateMessages]);

  // Automatic Background Polling Timer (every 12 seconds)
  useEffect(() => {
    if (!hasCloudConfig || !activeCloudConfig?.sheetUrl) return;

    handleManualSyncCloud();
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    onSendMessage(newMsgData);

    if (activeCloudConfig?.sheetUrl && validateAppsScriptUrl(activeCloudConfig.sheetUrl)) {
      ...
      {activeTab === 'messenger' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
          ...
          <div className="lg:col-span-8 bg-white rounded-3xl border border-slate-200 shadow-sm flex flex-col h-[640px] overflow-hidden">
            ...
            {/* Cloud Sync Status / Offline Fallback Bar */}
            {!hasCloudConfig ? (
              <div className="px-4 py-2 bg-amber-50 border-b border-amber-200 text-amber-900 text-[11px] font-semibold flex items-center justify-between gap-2 shadow-xs">
                <div className="flex items-center gap-1.5 min-w-0 truncate">
                  <AlertTriangle className="w-3.5 h-3.5 text-amber-600 shrink-0" />
                  <span className="truncate">💡 Chưa kết nối Cloud Sheet, tin nhắn chỉ lưu trên thiết bị này.</span>
                </div>
                {isGVCN && onOpenGoogleSheetModal && (
                  <button
                    type="button"
                    onClick={onOpenGoogleSheetModal}
                    className="px-2.5 py-1 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-bold text-[10px] shrink-0 transition-colors shadow-xs"
                  >
                    Kết Nối Webhook
                  </button>
                )}
              ...
              <div className="px-4 py-1.5 bg-emerald-50 border-b border-emerald-200 text-emerald-900 text-[11px] font-semibold flex items-center justify-between gap-2 shadow-xs">
                <div className="flex items-center gap-2 min-w-0 truncate">
                  <span className="relative flex h-2 w-2 shrink-0">
                    <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                    <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
                  </span>
                  <span className="truncate">🟢 Đã kết nối Cloud Chat Trực Tuyến (Đồng bộ 12s)</span>
                  {lastSyncTime && (
                ...
                <button
                  ...
                  className="px-2.5 py-1 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-[10px] shrink-0 transition-all flex items-center gap-1 cursor-pointer disabled:opacity-50"
                  title="Lấy tin nhắn mới nhất từ Cloud ngay lập tức"
                >
                  <RotateCcw className={`w-3 h-3 ${isCloudSyncing ? 'animate-spin' : ''}`} />
                  <span>{isCloudSyncing ? 'Đang tải...' : 'Đồng bộ ngay'}</span>
                </button>
              </div>
            )}
