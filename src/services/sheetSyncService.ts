...
/**
 * Sends a single chat message to Google Apps Script Cloud Webhook.
 */
export const sendChatMessageToCloud = async (
  ...
  try {
    ...
    console.warn('sendChatMessageToCloud warning:', err);
    return false;
  }
};

/**
 * Fetches latest chat messages from Google Apps Script Cloud Webhook.
 */
export const fetchChatMessagesFromCloud = async (
  scriptUrl: string | undefined,
  classId: string
): Promise<any[]> => {
  if (!scriptUrl || !validateAppsScriptUrl(scriptUrl)) {
  ...
  try {
    const delimiter = scriptUrl.includes('?') ? '&' : '?';
    const url = `${scriptUrl}${delimiter}action=get_chat_messages&classId=${encodeURIComponent(classId)}`;
    const response = await fetch(url);
    const result = await response.json();

    if (result && (result.status === 'success' || result.success === true) && Array.isArray(result.messages)) {
    ...
    if (result && result.data && Array.isArray(result.data.chat_messages)) {
      return result.data.chat_messages;
    }
    return [];
  } catch (err) {
    console.warn('fetchChatMessagesFromCloud warning:', err);
    return [];
  }
};
